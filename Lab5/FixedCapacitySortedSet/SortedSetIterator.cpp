...
SortedSetIterator::SortedSetIterator(const SortedSet& set) : sortedSet(set)
{
	...
	/// populate the stack with the leftmost nodes
...
/// best case: theta(1): the tree is degenerate with all children on the right side, so inside the while loop only the
/// root is pushed to the stack.
...
/// best case: theta(1): when the the current node is not the root of a subtree and we simply go up the tree
