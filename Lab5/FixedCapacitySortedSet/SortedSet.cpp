...
SortedSet::SortedSet(Relation r, int capacity) {
...
bool SortedSet::add(TComp elem) {
    ...
    /// we cannot add elements if the set is already full.
    if(this->isFull())
        return false;

	/// if there is still space, we insert it at the right position
...
/// best case: theta(1): the tree is full and we don't continue adding or the root already contains the element that is to be added
...
/// best case: theta(1), when we remove the root and it has at most one descendant, so the first while loop is executed only once
/// worst case: theta(n+n), when we remove the root and it has 2 descendants, so one n comes from we parsing the whole
/// left subtree to find the maximum, and the second n comes from parsing the tree again to remove the node of the maximum.
/// BC != WC => total O(n+n)
...
/// best case: theta(1): when the element that is searched is the root.
/// worst case: theta(n): the tree is degenerate and all nodes have their child on the same side, either left or right,
