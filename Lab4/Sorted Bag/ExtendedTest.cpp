...
bool conditionEven(TComp e){
    if(e%2 == 0)
        return true;
    return false;
}

bool conditionOdd(TComp e){
    return !conditionEven(e);
}
...
void testFilter(){
    ...
    assert(sb.size() == 100);
    sb.filter(conditionEven);
    assert(sb.size() == 50);

    SortedBagIterator bagIterator = sb.iterator();
    while(bagIterator.valid()){
        TElem current = bagIterator.getCurrent();
        assert(conditionEven(current) == true);
        bagIterator.next();
    }

    sb.filter(conditionOdd);
    assert(sb.isEmpty());
